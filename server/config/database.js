...
connection.connect((error) => {
  if (error) {
    ...
    let sqlQuery = `CREATE DATABASE IF NOT EXISTS ${dbOptions.database}`;
    connection.query(sqlQuery, (err, docs) => {
      if (err) {
        console.error({ error: err.message });
        return;
      }

      let sqlQuery = "call freedbtech_hrDB.createEmployeeTable();";
      connection.query(sqlQuery, (err) => {
        ...
        console.log({ message: "created SP!" });
      });
      sqlQuery = "call freedbtech_hrDB.createContractsTable();";
      connection.query(sqlQuery, (err) => {
        if (err) {
          console.error({ error: err.message });
          return;
